...
{
    public class PartationController : OriginBaseController
    {
        private readonly IPartationComponent _partationComponent;
        private readonly IPartationTypeComponent _partationTypeComponent;
        public PartationController(IPartationComponent partationComponent, IPartationTypeComponent partationTypeComponent)
        {
        ...
        {
            ViewBag.partationTypes = _partationTypeComponent.GetList(string.Empty);
            return View();
        }

        [HttpPost]
        public ActionResult Create(PartationViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    if (model.Id != null && model.Id != Guid.Empty)
                        _partationComponent.Update(model);
                    else
                        _partationComponent.Create(model);
        ...
        public ActionResult PageData(IDataTablesRequest request)
        {
            ...
            var data = _partationComponent.GetPageData(request.Search.Value, request.Start, request.Length, orderColumsName, orderColumsDirect);
        ...
        public ActionResult Get(Guid id)
        {
            try
            {
                ...
                {
                    var mdel = _partationComponent.Get(id);
                    mdel.Id = mdel.Id;
        ...
        public ActionResult Deactive_Activate(Guid id)
        {
            try
            {
                if (id != null && id != Guid.Empty)
                {
                    _partationComponent.Deactive_Activate(id);

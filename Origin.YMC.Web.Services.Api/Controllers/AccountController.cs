...
using Origin.YMC.Web.Services.Api.Models;
using Origin.YMC.Web.Services.Api.Providers;
using Origin.YMC.Web.Services.Api.Results;
...
namespace Origin.YMC.Web.Services.Api.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            var user = new Business.Entities.Domain.ApplicationUser() { UserName = model.Email, Email = model.Email };
        ...
        public async Task<HttpResponseMessage> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            if (result.Succeeded)
                ...
                return Request.CreateResponse(HttpStatusCode.InternalServerError, string.Join(" ", result.Errors));
        ...
        [Route("login")]
        [HttpPost]
        [ResponseType(typeof(AccessTokenViewModel))]
        public async Task<HttpResponseMessage> Login(LoginViewModel model)
        {
            var user = await UserManager.FindByEmailAsync(model.Email);
            var isAuthenticated = await UserManager.CheckPasswordAsync(user, model.Password);
            if (isAuthenticated)
            {
                var identity = await UserManager.CreateIdentityAsync(user, Startup.OAuthOptions.AuthenticationType);
                var ticket = new AuthenticationTicket(identity, new AuthenticationProperties());
                var currentUtcOffset = new Microsoft.Owin.Infrastructure.SystemClock().UtcNow;
                var tokenExpiryDays = int.Parse(ConfigurationManager.AppSettings["usertokenexpiry"]);
                var tokenExpirationTimeSpan = TimeSpan.FromDays(tokenExpiryDays);

                ticket.Properties.IssuedUtc = currentUtcOffset;
                ticket.Properties.ExpiresUtc = currentUtcOffset.Add(tokenExpirationTimeSpan);
                ...
                var result = new AccessTokenViewModel
                {
                    ...
                    TokenType = "bearer",
                    ExpiresIn = tokenExpirationTimeSpan.TotalSeconds,
                ...
                return Request.CreateResponse(HttpStatusCode.InternalServerError, "login failure.");
